...
const mainImageOutput = document.querySelector('.feature img');
const captionImgOutput = document.querySelector('.feature .caption');
const arrayOfImages = document.querySelector('.thumbnails ul').children;
let selectedImgIndex = 0;

const setMainImage = (index) => {
  mainImageOutput.setAttribute('src', `images/${content[index]['image']}`);
  captionImgOutput.textContent = content[index]['caption'];
};

const setSelectedClass = (el) => {
  el.classList.add('selected');
};

const slider = (e) => {
  const arrow = e.target.closest('.arrow');
  const thumbnailsImg = e.target.closest('.thumbnails__img');

  if (arrow) {
    arrayOfImages[selectedImgIndex].classList.remove('selected');

    if (arrow.classList.contains('right')) {
      if (selectedImgIndex < arrayOfImages.length - 1) {
        ++selectedImgIndex;
      } else {
        selectedImgIndex = 0;
      }
    ...
    if (arrow.classList.contains('left')) {
      if (selectedImgIndex > 0) {
        ...
        selectedImgIndex = arrayOfImages.length - 1;
      }
    }
  }
  ...
  if (thumbnailsImg) {
    arrayOfImages[selectedImgIndex].classList.remove('selected');
    selectedImgIndex = e.target.dataset.ind;
    // setSelectedClass(e.target);
    // setMainImage(selectedImgIndex);
  }

  setSelectedClass(arrayOfImages[selectedImgIndex]);
  setMainImage(selectedImgIndex);
};

document.body.addEventListener('click', slider);
